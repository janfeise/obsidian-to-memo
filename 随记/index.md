...
function createMessageItem(msg, avatarMap) {
    ...
    chatItem.className = "chat-item" + (msg.sender === "me" ? " chat-item-me" : "")
...
`
        ...
        let cache = window.messageCache.get(dailyFilePath)
        if (!cache) {
            cache = {
                mtime: Date.now(),
                messages: []
        ...
        const newChatItem = createMessageItem(newMsg, avatarMap)

        chatContainer.prepend(newChatItem)

        cache.messages.push(newMsg)

        window.messageCache.set(dailyFilePath, cache)
        ...
        try {
            ...
            if (existingFile) {
                // 文件已存在：追加内容
                app.vault.append(existingFile, "\n\n" + messageBlock.trim() + "\n")
                    .then(() => {
                        // 写入成功后，用文件系统真实的 mtime 更新缓存
                        const latestFile = app.vault.getAbstractFileByPath(dailyFilePath)
                        if (latestFile) {
                            cache.mtime = latestFile.stat.mtime
                            window.messageCache.set(dailyFilePath, cache)
                        }
                ...
                // 文件不存在：创建新文件
                app.vault.create(dailyFilePath, messageBlock.trim() + "\n")
                    .then(() => {
                        // 写入成功后，用文件系统真实的 mtime 更新缓存
                        const latestFile = app.vault.getAbstractFileByPath(dailyFilePath)
                        if (latestFile) {
                        ...
                        // ===== 更新 dailyFiles，并保持降序 =====
                        if (!dailyFiles.some(f => f.path === dailyFilePath)) {
                            dailyFiles.push(latestFile)
